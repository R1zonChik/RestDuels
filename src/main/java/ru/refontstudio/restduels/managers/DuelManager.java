...
public class DuelManager {
    ...
    /**
     * Проверяет, находится ли игрок в очереди на дуэль указанного типа
     * @param playerId UUID игрока
    ...
    public void queuePlayer(Player player, DuelType type) {
        ...
        // Проверка на нахождение в очереди (любого типа)
        for (DuelType queueType : DuelType.values()) {
            if (queuedPlayers.get(queueType).contains(playerId)) {
                player.sendMessage(ColorUtils.colorize(
                        plugin.getConfig().getString("messages.prefix") +
                                "&cВы уже находитесь в очереди! Используйте /duel cancel для отмены."));
                return;
            }
        }
        ...
        if (frozenPlayers.contains(playerId)) {
            player.sendMessage(ColorUtils.colorize(
                    plugin.getConfig().getString("messages.prefix") +
                            "&cВы уже ожидаете начала дуэли! Используйте /duel cancel для отмены."));
            return;
        }

        // Проверка на нахождение в очереди на арену
        for (QueueEntry entry : arenaQueue) {
            if (entry.player1Id.equals(playerId) || entry.player2Id.equals(playerId)) {
                player.sendMessage(ColorUtils.colorize(
                        plugin.getConfig().getString("messages.prefix") +
                                "&cВы уже ожидаете свободную арену! Используйте /duel cancel для отмены."));
                return;
            }
        }

        // Сохраняем местоположение игрока, только если он НЕ в мире дуэлей
        saveOriginalLocation(player);

        // Сохраняем оригинальный инвентарь игрока для всех типов дуэлей
    ...
    private void freezePlayersBeforeDuel(Player player1, Player player2, DuelType type, Arena arena) {
        ...
        // Добавляем игроков в список замороженных
        ...
        String confirmMessage = ColorUtils.colorize(
                plugin.getConfig().getString("messages.prefix") +
                        "&eНайден соперник! Начало дуэли через &c8 секунд&e.");
    ...
    public void returnPlayer(Player player, boolean restoreInventory) {
        ...
        // ДОБАВЛЕНО: Разрешаем использование команд
        ...
        // ВАЖНО: Используем originalWorldLocations вместо playerLocations
    ...
    public void endDuel(Duel duel, UUID winnerId, boolean isDraw) {
        ...
        if (duel.getType() == DuelType.RANKED) {
            ...
            if (isDraw) {
                ...
                if (player1 != null && player1.isOnline()) {
                    // ДОБАВЛЕНО: Разблокируем команды для игрока при ничьей
                ...
                BukkitTask delayedTask = Bukkit.getScheduler().runTaskLater(plugin, () -> {
                    ...
                    // Очищаем арену от стрел и других ресурсов
                    if (arena != null) {
                        // Авто��атическое восстановление арены
                        plugin.getRestoreManager().restoreArenaAreas(arena.getId());
                ...
                // ИЗМЕНЕНО: Отправляем кнопки ПОСЛЕ добавления задач в delayedReturnTasks
                if (player1 != null && player1.isOnline()) {
                ...
                if (winner != null && winner.isOnline()) {
                    ...
                    // ДОБАВЛЕНО: Сразу разблокируем команды для победителя
                    if (winner.hasMetadata("restduels_blocked_commands")) {
                        winner.removeMetadata("restduels_blocked_commands", plugin);
                    }
                    ...
                    try {
                        ...
                        if (commandBlockerClass != null) {
                            ...
                            if (getInstanceMethod != null) {
                                ...
                                java.lang.reflect.Method removePlayerMethod = commandBlocker.getClass().getMethod("removePlayer", UUID.class);
                                if (removePlayerMethod != null) {
                                    removePlayerMethod.invoke(commandBlocker, winnerUUID);
                                }
                            }
                        }
                    } catch (Exception e) {
                    ...
                    // ИЗМЕНЕНО: Затем сохраняем задачу в список
                    delayedReturnTasks.put(winnerUUID, winnerTask);

                    // ИЗМЕНЕНО: И только потом отправляем кнопку досрочного возврата
    ...
    private void sendEarlyReturnButton(Player player) {
        ...
        if (!delayedReturnTasks.containsKey(playerId)) {
            ...
            if (plugin.getConfig().getBoolean("debug", false)) {
                plugin.getLogger().info("Не удалось отправить кнопку досрочного возврата игроку "
                        + player.getName() + ": нет отложенной задачи возврата");
            }
            return;
        }
        ...
        if (isPlayerInDuel(playerId)) {
            ...
            if (plugin.getConfig().getBoolean("debug", false)) {
                plugin.getLogger().info("Не удалось отправить кнопку досрочного возврата игроку "
                        + player.getName() + ": игрок в активной дуэли");
            }
            return;
        }
        ...
        // ДОБАВЛЕНО: Выводим отладочную информацию об успешной отправке
        if (plugin.getConfig().getBoolean("debug", false)) {
            plugin.getLogger().info("Кнопка досрочного возврата отп��авлена игроку " + player.getName());
        }
    }
    ...
    /**
     * Размораживает игрока и отменяет дуэль при выходе с сервера
     */
    ...
    private void sendCancelButton(Player player) {
        ...
        // Отправляем сообщение для отмены дуэли
        player.sendMessage(ColorUtils.colorize(
                plugin.getConfig().getString("messages.prefix") +
                        "&eДля отмены дуэли нажмите на кнопку ниже в течение &c" +
                        cancelTime + " &eсекунд."));

        // Получаем текст кнопки из конфига и форматируем его
        String cancelText = plugin.getConfig().getString("messages.cancel-button", "&c[Выйти с дуэли]");
        ...
        // Создаем JSON для кликабельного текста
        String json = "{\"text\":\"" + cancelText + "\",\"clickEvent\":{\"action\":\"run_command\",\"value\":\"/duel cancel\"},\"hoverEvent\":{\"action\":\"show_text\",\"value\":\"Нажмите, чтобы отменить дуэль\"}}";
